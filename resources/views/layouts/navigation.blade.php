...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden sm:flex sm:items-center sm:-my-px space-x-8">
                    ...
                    <div x-data="{drop:false}" @mouseenter="drop=true" @mouseleave="drop=false" @click.outside="drop=false" class="relative sm:flex sm:items-center">
                        <x-nav-link class="h-16" :href="route('cart.index')" :active="request()->routeIs('cart.*')" aria-haspopup="true" x-bind:aria-expanded="drop ? 'true' : 'false'">Carrinho</x-nav-link>
                        <div x-cloak x-show="drop" class="absolute left-0 top-full mt-0 w-56 rounded-md bg-white shadow border z-50">
                            <div class="py-1">
                                @auth
                                @if(Route::has('cliente.pedidos.index'))
                    ...
                    <div x-data="{drop:false}" @mouseenter="drop=true" @mouseleave="drop=false" @click.outside="drop=false" class="relative sm:flex sm:items-center">
                        <x-nav-link class="h-16" :href="route('adm.produtos.index')" :active="request()->routeIs('adm.produtos.*')" aria-haspopup="true" x-bind:aria-expanded="drop ? 'true' : 'false'">Catálogo</x-nav-link>
                        <div x-cloak x-show="drop" class="absolute left-0 top-full mt-0 w-56 rounded-md bg-white shadow border z-50">
                            <div class="py-1">
                    ...
                    {{-- SUPORTE (todos logados) --}}
                    @if($user?->hasRole(['cliente','adm','gerente']) && Route::has('cliente.sac.index'))
                    <x-nav-link class="h-16" :href="route('cliente.sac.index')" :active="request()->routeIs('cliente.sac.*')">Suporte</x-nav-link>
                    @endif
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden" x-data="{ cartOpen:false, catalogOpen:false }">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.*')" @click.prevent="cartOpen=!cartOpen" x-bind:aria-expanded="cartOpen ? 'true' : 'false'">Carrinho</x-responsive-nav-link>
            ...
            @if($isStaff && Route::has('adm.produtos.index'))
            <x-responsive-nav-link :href="route('adm.produtos.index')" :active="request()->routeIs('adm.produtos.*')" @click.prevent="catalogOpen=!catalogOpen" x-bind:aria-expanded="catalogOpen ? 'true' : 'false'">Catálogo</x-responsive-nav-link>
            ...
            {{-- SUPORTE --}}
            @if($user?->hasRole(['cliente','adm','gerente']) && Route::has('cliente.sac.index'))
            <x-responsive-nav-link :href="route('cliente.sac.index')" :active="request()->routeIs('cliente.sac.*')">Suporte</x-responsive-nav-link>
            @endif
